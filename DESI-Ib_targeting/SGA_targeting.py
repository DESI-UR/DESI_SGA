...
def xx(ra0, dec0, ra, dec):

    dra = ra - ra0

    return np.cos(dec)*np.sin(dra)/(np.sin(dec0)*np.sin(dec) + np.cos(dec0)*np.cos(dec)*np.cos(dra))


def yy(ra0, dec0, ra, dec):

    dra = ra - ra0

    return (np.cos(dec0)*np.sin(dec) - np.sin(dec0)*np.cos(dec)*np.cos(dra))/(np.sin(dec0)*np.sin(dec) + np.cos(dec0)*np.cos(dec)*np.cos(dra))


def radec_to_xy(ra, dec, ra0=0, dec0=0, x0=0, y0=0, xscale=1, yscale=1):
    '''
    Converts RA/DEC (degrees) x/y (pixels) position using the TAN Gnomonic 
    projection system.

    From https://github.com/AngusWright/LAMBDAR/blob/master/R/radec.to.xy.R
    ...
    X = x0 + xx(ra0, dec0, ra, dec)/np.tan(xscale)
    Y = y0 + yy(ra0, dec0, ra, dec)/np.tan(yscale)
...
class GalaxyMarker(object):
    ...
    def __init__(self, input_filename):
        ...
        self.out_filename = "SGA2025_off-axis_targets_" + file_index + ".txt"
