'''
This file is the same as joint-Coma-0pt_Fuji-ITFR_KAD_varyV0-perpdwarfs.ipynb, 
but should only be used to generate the final plots; it does not contain any of 
...
# Best-fit
#-------------------------------------------------------------------------------
temp_infile = open('cov_ab_fuji_joint_ITFR_varyV0-perpdwarfs2_KAD.pickle', 'rb')
cov_ab, itfr_samples, tfr_samples, V0 = pickle.load(temp_infile)
temp_infile.close()
...
ax2.plot(xvals - sig_fit[0], yvals[0], "k--", lw=1.3)
ax2.plot(xvals + sig_fit[0], yvals[0], "k--", lw=1.3)
